...
const MarketingServices = lazy(() => import("./pages/MarketingServices.tsx"));
const MarketingServiceDetail = lazy(() => import("./pages/MarketingServiceDetail.tsx"));
...
// Lazy: the gate reads the user profile through Supabase, which marketing
// pages shouldn't have to download.
const OnboardingGate = lazy(() =>
  import("./components/auth/OnboardingGate.tsx").then((m) => ({ default: m.OnboardingGate })),
);
...
/**
 * The whole route table. Shared by the browser app (BrowserRouter, in App.tsx)
 * and the build-time prerenderer (StaticRouter, in src/prerender.tsx), so the
 * static HTML is always rendered from exactly the routes the app serves.
 */
export const AppRoutes = () => (
  <Routes>
    ...
    <Route element={<MarketingShell />}>
      ...
      <Route path="/marketing/:slug" element={<MarketingServiceDetail />} />
